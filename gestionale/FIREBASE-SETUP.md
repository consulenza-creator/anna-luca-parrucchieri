...
- [x] Disponibilità reale: gli slot mostrati ai clienti ora controllano le prenotazioni Firestore esistenti invece di un algoritmo pseudo-casuale. Per non far leggere ai clienti non autenticati i dati personali delle altre prenotazioni, è stata creata una collezione separata `disponibilita/{id}` (stesso id della prenotazione, ma solo data/ora/operatore/durata — niente nome, telefono, email, note) scritta insieme a ogni prenotazione e cancellata da Anna e Luca quando annullano un appuntamento. Resta un margine teorico molto piccolo: se due persone confermano lo stesso slot nello stesso istante esatto, in rari casi potrebbero sovrapporsi (il controllo è lato client, non una transazione atomica lato server) — per i volumi di un salone è un rischio trascurabile, ma è onesto segnalarlo.
- [ ] **Azione richiesta:** pubblicare la versione aggiornata di `firestore.rules` (contiene le nuove regole per `disponibilita`) in Firebase Console → Firestore Database → Regole → incolla il contenuto di `gestionale/firestore.rules` → Pubblica. Finché non viene fatto, il controllo di disponibilità reale resta inattivo (il sito continua a funzionare, ma non blocca ancora gli orari già occupati).
